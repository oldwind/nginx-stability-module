...
static ngx_command_t  ngx_http_anti_module_commands[] = {
    ...
    { ngx_string("upstream_static_size"),
        ...
        offsetof(ngx_http_anti_conf_t, size),
        NULL },
    ...
    { ngx_string("upstream_static_active_time"),
        ...
        ngx_conf_set_size_slot,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_http_anti_conf_t, active_time),
        NULL },    
...
ngx_http_anti_handler(ngx_http_request_t *r){
    ...
    // 获取配置信息
    ngx_http_anti_conf_t * uscf;
    uscf = ngx_http_get_module_loc_conf(r, ngx_http_anti_module);
...
ngx_http_anti_create_loc_conf(ngx_conf_t *cf) {
    ngx_http_anti_conf_t *uscf;
    uscf = ngx_pcalloc(cf->pool, sizeof(ngx_http_anti_module));
    if (uscf == NULL) {
        return NULL;
    }
